...
axiosInstance.interceptors.request.use(
  (config: any) => {
    const token = localStorageSerive.getLocalStorage('token');
    if (token) {
...
const logout = () => {
  localStorageSerive.clearLocalStorage();
...
axiosInstance.interceptors.response.use(
  ...
  (error: any) => {
    ...
    const refreshToken = localStorageSerive.getLocalStorage('refreshToken');
    ...
    return Axios.post(`${process.env.NEXT_PUBLIC_ENV_DOMAIN}/auth/refresh`)
      .then((res) => {
        if (res.status === 200) {
          const { data } = res.data;
          localStorageSerive.setLocalStorage('token', data.access_token);
          localStorageSerive.setLocalStorage('refreshToken', data.refresh_token);
